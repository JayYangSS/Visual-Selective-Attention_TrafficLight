...
void BoxDetect(Mat src_test,HOGDescriptor &myHOG,vector<Rect> &found_filtered,bool HORZ)
{
	...
	if(HORZ)
		myHOG.detectMultiScale(src_test, found,-0.77,Size(4,4), Size(0,0), 1.05, 2);//改成-0.75后，对10.23号采集的横向交通灯检测效果好
	                                                                                //在release模式下，第四个参数使用Size(5,5)没有问题，debug模式下不行，改成Size(4,4)才好使
	else//竖直检测使用
		myHOG.detectMultiScale(src_test, found,0.8,Size(4,4), Size(0,0), 1.05, 2);//对图片进行多尺度行人检测
